...
//connecting database
//Client is sending request at port 3000 to Server
...
app.post("/update", function(req, res){ 
    if ( req.body.updateto == ""){res.redirect("/contacts"); }
    else { query = "update contacts set phone='" + req.body.updateto + "' where ID=" + id + " and name='" + names[req.body.toupdate] + "'";
    connect.query(query, function (err, result) {
        if (err) throw err;
        res.redirect("/contacts");
    });
...
//Server is listening to the Client's request at the port 3000
